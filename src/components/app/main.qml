...
ApplicationWindow {
            ...
            ColumnLayout {
                ...
                GroupBox {
                    ...
                    ColumnLayout {
                        ...
                        Text {
                            text: "Download: " + formatNetSpeed(uiManager ? uiManager.netRxKbps : 0)
                            color: "#1565C0"
                            font.pixelSize: 12
                        }
                        ...
                        ProgressBar {
                            Layout.fillWidth: true
                            value: Math.min(uiManager ? uiManager.netRxKbps : 0, 100)
                            to: 100
                        }

                        Text {
                            text: "Upload: " + formatNetSpeed(uiManager ? uiManager.netTxKbps : 0)
                            color: "#E65100"
                            font.pixelSize: 12
                        }
                        ...
                        ProgressBar {
                            Layout.fillWidth: true
                            value: Math.min(uiManager ? uiManager.netTxKbps : 0, 100)
                            to: 100
                        }
                    }
                }
    ...
    Rectangle {
        ...
        Row {
            ...
            Text {
                text: "Net: " + (uiManager ? formatNetSpeed(uiManager.netTxKbps) : "0 KB/s") + " \u2191"
                color: "#81C784"
                font.pixelSize: 12
            }
    ...
    function formatNetSpeed(kbps) {
        if (kbps < 1) return Math.round(kbps * 100) / 100 + " KB/s"
        if (kbps < 1024) return Math.round(kbps * 10) / 10 + " KB/s"
        return Math.round(kbps / 10.24) / 100 + " MB/s"
    }
}
