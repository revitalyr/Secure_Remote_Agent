...
#include "core/remote_executor.h"
#include <QFileInfo>
#include <QDir>
#include <QDateTime>
...
void RemoteExecutor::onProcessFinished(int exitCode, QProcess::ExitStatus exitStatus) {
    ...
    result["stdout"] = currentCommandOutput;
    result["stderr"] = currentCommandError;
...
void RemoteExecutor::onProcessReadyReadStandardError() {
    if (currentProcess) {
        QString errorText = QString::fromUtf8(currentProcess->readAllStandardError());
        currentCommandError += errorText;
        emit commandError(errorText);
    }
}
