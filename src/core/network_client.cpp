...
void NetworkClient::postJson(const QUrl& url, const QJsonObject& obj) {
    ...
    if (!authenticationToken.isEmpty()) {
        req.setRawHeader("Authorization", ("Bearer " + authenticationToken).toUtf8());
        req.setRawHeader("X-API-Key", authenticationToken.toUtf8());
    }
    configureSsl(req);
...
void NetworkClient::attemptReconnect() {
    if (isFileTransfer && !lastFilePathName.isEmpty()) {
        postFile(lastUrl, lastFilePathName);
    } else {
