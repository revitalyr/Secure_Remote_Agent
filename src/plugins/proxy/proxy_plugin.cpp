...
#include "plugins/proxy/proxy_plugin.h"
#include <QJsonObject>
...
bool ProxyPlugin::registerCommand(const CommandDescriptor& descriptor) {
    QMutexLocker locker(&m_mutex);
...
bool ProxyPlugin::unregisterCommand(const QString& commandName) {
    QMutexLocker locker(&m_mutex);
    return m_commands.remove(commandName) > 0;
}

QVariantMap ProxyPlugin::executeCommand(const QString& commandName, const QVariantMap& params) {
    QMutexLocker locker(&m_mutex);
...
QStringList ProxyPlugin::getRegisteredCommands() const {
    QMutexLocker locker(&m_mutex);
    return m_commands.keys();
}

CommandDescriptor ProxyPlugin::getCommandDescriptor(const QString& commandName) const {
    QMutexLocker locker(&m_mutex);
