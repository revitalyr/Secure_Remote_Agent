...
#include "plugins/scraper/scraper_plugin.h"
...
bool ScraperPlugin::initialize(const QVariantMap& config) {
    ...
    if (m_status != PluginStatus::Unloaded) {
        lastError = "Plugin already initialized";
        return false;
    }
...
void ScraperPlugin::start() {
    ...
    if (m_status != PluginStatus::Loaded) {
        lastError = "Plugin not initialized";
        return;
    }
...
bool ScraperPlugin::registerCommand(const CommandDescriptor& descriptor) {
    QMutexLocker locker(&m_mutex);
    if (m_commands.contains(descriptor.commandName))
        return false;
    m_commands[descriptor.commandName] = descriptor;
    return true;
}

bool ScraperPlugin::unregisterCommand(const QString& commandName) {
    QMutexLocker locker(&m_mutex);
    return m_commands.remove(commandName) > 0;
}

QVariantMap ScraperPlugin::executeCommand(const QString& commandName, const QVariantMap& params) {
    QMutexLocker locker(&m_mutex);
    if (!m_commands.contains(commandName))
        return {{"error", "Command not found: " + commandName}};
    if (m_commands[commandName].callback)
        return m_commands[commandName].callback(params);
    return {{"error", "No callback registered for command: " + commandName}};
}

QStringList ScraperPlugin::getRegisteredCommands() const {
    QMutexLocker locker(&m_mutex);
    return m_commands.keys();
}

CommandDescriptor ScraperPlugin::getCommandDescriptor(const QString& commandName) const {
    QMutexLocker locker(&m_mutex);
